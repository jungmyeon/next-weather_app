...
import { convertMeterToKilometers } from "@/utils/convertMetersToKilometers";
import { convertWindSpeed } from "@/utils/convertWindSpeed";
import { fromUnixTime, format, parseISO } from "date-fns";
import Gpt from "@/components/gpt/gpt";

// https://api.openweathermap.org/data/2.5/forecast?q=seoul&appid=5ddc138921c7c784ae4714a921267500&cnt=56
...
export default function Home() {
  ...
  // 백그라운드 작업 또는 타이머는 useEffect에서, 재실행은 place값이 바뀔때만
  useEffect(()=>{
    fetchWeatherData();
  },[place])
        ...
        <section className={styles.spaceY4}>
          ...
          <div className={styles.todaySecondBox}>
            ...
            <ListBox className="box-style4">
                <TodayDetail
                visibility={convertMeterToKilometers(todayData?.visibility ?? 10000)}
                ...
                <strong>일기예보 (자정 기준)</strong>
                  {
                    filteredList && filteredList.map((item,index)=>{
                      return(
                        ...
                        temp_main={item.main.temp_min ?? 0}
                        temp_max={item.main.temp_max ?? 0}
                        description={item.weather[0].description ?? 0}
                        visibility={convertMeterToKilometers(item?.visibility ?? 10000)}
...
// Name 입력 후 생성
// secret Key 백업
// 우측 상단의 Setting 에서 Billing 접속해 잔여요금확인
// payment methods 에서 카드 추가
// 잔여요금 0원이면 add payment details 에서 충전
// 카드등록하고 충전 ( 자동충전 체크박스 해제)
// (첫 카드 등록시 5달러 제공)